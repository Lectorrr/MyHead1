(function($, undefined) {
	...
	function AceAjax(contentArea, options) {
		...
		var update_breadcrumbs = options.update_breadcrumbs
				|| options.update_breadcrumbs === undefined;
		var update_title = options.update_title
				|| options.update_breadcrumbs === undefined;
		var update_active = options.update_active
				|| options.update_breadcrumbs === undefined;
		var close_active = options.close_active || false;
		var max_load_wait = options.max_load_wait || false;
		var target = options.target || "page";
		var working = false;
...
// 判断如何打开这个页面
		...
		this.getUrl = function(url, hash, manual_trigger) {
			if (working) {
				// 要快速响应，应该停止之前的请求，快速的处理现在的请求，因为地址已经变了
			...
			if(target == "ifContent"){
				$contentArea.empty().html("<iframe id=\"ifContent\" style=\"display:block;\" src=\""+url+"\" frameborder='0' width=100% ></iframe>");
				$("#ifContent").load(function(){  // 在这里load判断加载函数
					try{
						var ifm= document.getElementById("ifContent"); 
						var subWeb = document.frames ? document.frames["ifContent"].document : ifm.contentDocument; 
				...
				$.ajax({
					...
					.error(function(result) {
						self.stopLoading(true);
						// 处理错误情况
						$contentArea.empty().html(result.responseText);
					...
					.done(
							function(result) {
								...
								if (typeof update_active === "function") {
									link_element = update_active.call(null,
											hash, url)
								} else {
									if (update_active === true) {
										...
										if (link_element.length > 0) {
											...
											if (nav.length > 0) {
												...
												nav
														.closest(
																".sidebar[data-sidebar-scroll=true]")
														.each(
																function() {
																	var $this = $(this);
																	$this
																			.ace_sidebar_scroll("reset");
																	if (manual_trigger) {
																		$this
																				.ace_sidebar_scroll("scroll_to_active")
																	}
																})
											}
										}
								...
								if (typeof update_breadcrumbs === "function") {
									link_text = update_breadcrumbs.call(null,
											hash, url, link_element);
								} else {
									if (update_breadcrumbs === true
											&& link_element != null
								...
								if (typeof update_title === "function") {
									update_title.call(null, hash, url,
											link_text)
								} else {
		...
		this.startLoading = function() {
			...
			working = true;
			$contentArea.css("opacity", 0.25).prevAll(".ajax-loading-overlay")
					.remove();
			$(
					'<div class="ajax-loading-overlay"><i class="ajax-loading-icon fa fa-spin '
							+ loading_icon + '"></i> ' + loading_text
							+ "</div>").insertBefore(contentArea);
		...
		this.stopLoading = function(stopNow) {
			if (stopNow === true) {
				working = false;
				$contentArea.css("opacity", 1).prevAll(".ajax-loading-overlay")
						.remove();
				if (loadTimer != null) {
				...
				link_element
						...
						.each(
								function() {
									...
									link_clone
											.find(
													"i,.fa,.glyphicon,.ace-icon,.menu-icon,.badge,.label")
											.remove();
		...
		$(window).off("hashchange.ace_ajax").on("hashchange.ace_ajax",
				function(e, manual_trigger) {
