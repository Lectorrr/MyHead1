		...
		return this
			.each(function () {
				...
				var ts = this, grid = {
					...
					dragMove: function (x) {
						if (this.resizing) {
							var diff = x.pageX - this.resizing.startX, h = this.headers[this.resizing.idx], newWidth = p.direction === "ltr" ? h.width
							+ diff
								: h.width - diff, hn, nWn;
							if (newWidth > 33) {
								this.curGbox.css({
					...
					populateVisible: function () {
						...
						if (ttop > 0) {
							...
							npage = (parseInt((scrollTop + dh) / div, 10) || 0)
								+ 2 - page;
							empty = true
						}
						if (npage) {
				...
				var gv = $("<div class='ui-jqgrid-view' role='grid'></div>"), isMSIE = $.jgrid.msie;
				...
				var sortkeys = ["shiftKey", "altKey", "ctrlKey"], intNum = function (val, defval) {
					...
					return val
										 ...
										 rdata) {
					var cm = ts.p.colModel[pos], cellAttrFunc, ral = cm.align, result = 'style="', clas = cm.classes, nm = cm.name, celp, acp = [];
					if (ral) {
					...
					if (rowInd === 0) {
						...
						if ($.isFunction(cm.cellattr)
							...
							cellAttrFunc = $.isFunction(cm.cellattr) ? cm.cellattr
								: $.jgrid.cellattr[cm.cellattr];
							celp = cellAttrFunc.call(ts, rowId, tv,
								rawObject, cm, rdata);
					...
					var p = ts.p, data = p.data, dataLength = data.length, i, j, cur, idn, idr, ccur, v, rd, localReader = p.localReader, colModel = p.colModel, cellName = localReader.cell, iOffset = (p.multiselect === true ? 1
							: 0)
						+ (p.subGrid === true ? 1 : 0)
						+ (p.rownumbers === true ? 1 : 0), br = p.scroll ? $.jgrid
						.randId()
						: 1, arrayReader, objectReader, rowReader;
					if (p.datatype !== "local"
					...
					idn = p.keyIndex === false ? ($
						.isFunction(localReader.id) ? localReader.id
						.call(ts, data) : localReader.id) : p.keyIndex;
					for (i = 0; i < dataLength; i++) {
						cur = data[i];
						idr = $.jgrid.getAccessor(cur, idn);
						if (idr === undefined) {
							if (typeof idn === "number"
								&& colModel[idn + iOffset] != null) {
								idr = $.jgrid.getAccessor(cur, colModel[idn
								+ iOffset].name)
							}
							if (idr === undefined) {
								idr = br + i;
								if (cellName) {
									ccur = $.jgrid.getAccessor(cur,
											cellName)
										|| cur;
									idr = ccur != null
									&& ccur[idn] !== undefined ? ccur[idn]
										: idr;
									ccur = null
								}
							}
						...
						rowReader = $.isArray(cur) ? arrayReader
							: objectReader;
					...
					var tabindex = "-1", restAttr = "", attrName, style = hide ? "display:none;"
						...
						ts).triggerHandler("jqGridRowAttr",
						[rd, cur, id]);
					if (typeof rowAttrObj !== "object") {
						rowAttrObj = $.isFunction(ts.p.rowattr) ? ts.p.rowattr
							.call(ts, rd, cur, id)
							: (typeof ts.p.rowattr === "string"
						&& $.jgrid.rowattr != null
						&& $
							.isFunction($.jgrid.rowattr[ts.p.rowattr]) ? $.jgrid.rowattr[ts.p.rowattr]
							.call(ts, rd, cur, id)
							: {})
					}
					...
					var self = $(ts), i, fpos, ir = 0, v, gi = ts.p.multiselect === true ? 1
						...
						: 0, idn, getId, f = [], F, rd = {}, xmlr, rid, rowData = [], cn = (ts.p.altRows === true) ? ts.p.altclass
						: "", cn1;
					...
					ts.p.userData = {};
					ts.p.page = intNum($.jgrid.getXmlData(xml, xmlRd.page),
						ts.p.page);
					ts.p.lastpage = intNum($.jgrid.getXmlData(xml,
						xmlRd.total), 1);
					ts.p.records = intNum($.jgrid.getXmlData(xml,
						xmlRd.records));
					...
					if (ts.p.gridview === true) {
						...
						if (ts.p.grouping) {
							...
							if (ts.p.treeGrid === true && fpos > 0) {
								...
								ts.firstElementChild.innerHTML += rowData
									.join("");
					...
					if (locdata) {
						while (ir < gl) {
							...
							rd[xmlid] = $.jgrid.stripPref(ts.p.idPrefix,
								rid);
							if (ts.p.grouping) {
						...
						if (ts.p.grouping) {
							...
							self.jqGrid("groupingRender", grpdata,
								ts.p.colModel.length, ts.p.page, rn);
							grpdata = null
						}
					}
					...
					var self = $(ts), ir = 0, v, i, j, f = [], cur, gi = ts.p.multiselect ? 1
						: 0, si = ts.p.subGrid === true ? 1 : 0, addSubGridCell, ni = ts.p.rownumbers === true ? 1
						: 0, arrayReader = orderedCols(gi + si + ni), objectReader = reader(frd), rowReader, len, drows, idn, rd = {}, fpos, idr, rowData = [], cn = (ts.p.altRows === true) ? ts.p.altclass
						: "", cn1;
					ts.p.page = intNum($.jgrid.getAccessor(data,
						dReader.page), ts.p.page);
					ts.p.lastpage = intNum($.jgrid.getAccessor(data,
						dReader.total), 1);
					ts.p.records = intNum($.jgrid.getAccessor(data,
						dReader.records));
					...
					while (i < len) {
						...
						if (idr === undefined) {
							if (typeof idn === "number"
								...
								idr = $.jgrid
									.getAccessor(cur, ts.p.colModel[idn
									+ gi + si + ni].name)
							}
							if (idr === undefined) {
								idr = br + i;
						...
						if (locdata || ts.p.treeGrid === true) {
							rd[locid] = $.jgrid.stripPref(ts.p.idPrefix,
								idr);
							ts.p.data.push(rd);
					...
					if (locdata) {
						while (ir < len && drows[ir]) {
							...
							if (idr === undefined) {
								if (typeof idn === "number"
									&& ts.p.colModel[idn + gi + si + ni] != null) {
									idr = $.jgrid.getAccessor(cur,
										ts.p.colModel[idn + gi + si
										+ ni].name)
								}
								if (idr === undefined) {
									idr = br + ir;
							...
							if (cur) {
								...
								for (j = 0; j < rowReader.length; j++) {
									rd[ts.p.colModel[j + gi + si + ni].name] = $.jgrid
										.getAccessor(cur, rowReader[j])
								}
								rd[locid] = $.jgrid.stripPref(
									ts.p.idPrefix, idr);
								if (ts.p.grouping) {
									groupingPrepare.call(self, rd, ir)
								}
					...
					if ((ts.p.search || ts.p.resetsearch) && ts.p.grouping
						&& ts.p.groupingView._locgr) {
						ts.p.groupingView.groups = [];
						var j, grPrepare = $.jgrid
							.getMethod("groupingPrepare"), key, udc;
					...
					if (ts.p.scroll) {
						...
						if (rh) {
							...
							if (ts.grid.bDiv.scrollTop == 0
								&& ts.p.page > 1) {
								ts.grid.bDiv.scrollTop = ts.p.rowNum
									* (ts.p.page - 1) * rh
							}
						}
					...
					if (!ts.grid.hDiv.loading) {
						...
						switch (dt) {
							...
							case "xmlstring":
								beginReq();
								dstr = typeof ts.p.datastr !== "string" ? ts.p.datastr
					...
					if (ts.p.rowList.length > 0) {
						...
						for (i = 0; i < ts.p.rowList.length; i++) {
							...
							str += '<option role="option" value="'
								+ strnm[0]
								+ '"'
								+ ((intNum(ts.p.rowNum, 0) === intNum(
									strnm[0], 0)) ? ' selected="selected"'
									: "") + ">" + strnm[1]
								+ "</option>"
						}
						str += "</select></td>"
					...
					if (ts.p.pgbuttons === true) {
						...
						pgl += "<td id='"
							...
							+ (ts.p.pgfirst ? "title='" + ts.p.pgfirst
							+ "'" : "")
							+ "><span class='ui-icon ui-icon-seek-first'></span></td>";
						pgl += "<td id='"
							+ po[1]
							+ "' class='ui-pg-button ui-corner-all' "
							+ (ts.p.pgprev ? "title='" + ts.p.pgprev
							+ "'" : "")
						...
						pgl += "<td id='"
							+ po[2]
							+ "' class='ui-pg-button ui-corner-all' "
							+ (ts.p.pgnext ? "title='" + ts.p.pgnext
							+ "'" : "")
							+ "><span class='ui-icon ui-icon-seek-next'></span></td>";
						pgl += "<td id='"
							+ po[3]
							+ "' class='ui-pg-button ui-corner-all' "
							+ (ts.p.pglast ? "title='" + ts.p.pglast
							+ "'" : "")
					...
					var splas, sort = "", cm = ts.p.colModel, fs = false, ls, selTh = ts.p.frozenColumns ? obj
						: ts.grid.headers[iCol].el, so = "";
					$("span.ui-grid-ico-sort", selTh).addClass(
						"ui-state-disabled");
					...
					if (cm[iCol].lso) {
						...
						cm[iCol].lso = so = cm[iCol].firstsortorder
							|| "asc"
					}
					if (so) {
						$("span.s-ico", selTh).show();
						$("span.ui-icon-" + so, selTh).removeClass(
							"ui-state-disabled");
					...
					if (ts.p.multiSort) {
						...
						var previousSelectedTh = ts.grid.headers[ts.p.lastsort].el, newSelectedTh = ts.p.frozenColumns ? obj
						...
						if (ts.p.frozenColumns) {
							ts.grid.fhDiv.find("span.ui-grid-ico-sort")
								.addClass("ui-state-disabled");
							ts.grid.fhDiv.find("th").attr("aria-selected",
								"false")
						}
						...
						ts.p.sortname = ts.p.colModel[idxcol].index
							|| index
					}
					if ($(ts).triggerHandler("jqGridSortCol",
							[ts.p.sortname, idxcol, ts.p.sortorder]) === "stop") {
						ts.p.lastsort = idxcol;
						return
					}
					if ($.isFunction(ts.p.onSortCol)) {
						if (ts.p.onSortCol.call(ts, ts.p.sortname, idxcol,
								ts.p.sortorder) === "stop") {
							ts.p.lastsort = idxcol;
							return
						}
					...
					var initwidth = 0, brd = $.jgrid.cell_width ? 0
						...
						ts.p.scrollOffset, 0), cw, hs = false, aw, gw = 0, cr;
					$
						.each(
							ts.p.colModel,
							function () {
								...
								if (ts.p.grouping && ts.p.autowidth) {
									...
									if (ind >= 0
										&& ts.p.groupingView.groupColumnShow.length > ind) {
					...
					var $th = $(ts.grid.headers[iCol].el), ret = [$th
						.position().left
					+ $th.outerWidth()];
					...
					ret.push($(ts.grid.bDiv).offset().top
				...
				ts.p.keyName = false;
				for (i = 0; i < ts.p.colModel.length; i++) {
					colTemplate = typeof ts.p.colModel[i].template === "string" ? ($.jgrid.cmTemplate != null
					&& typeof $.jgrid.cmTemplate[ts.p.colModel[i].template] === "object" ? $.jgrid.cmTemplate[ts.p.colModel[i].template]
						: {})
						: ts.p.colModel[i].template;
					ts.p.colModel[i] = $.extend(true, {}, ts.p.cmTemplate,
						colTemplate || {}, ts.p.colModel[i]);
					if (ts.p.keyName === false
				...
				var thead = "<thead><tr class='ui-jqgrid-labels' role='row'>", tdc, idn, w, res, sort, td, ptr, tbody, imgs, iac = "", idc = "", sortarr = [], sortord = [], sotmp = [];
				...
				if (ts.p.multiSort) {
					...
					for (i = 0; i < sortarr.length; i++) {
						...
						sortord[i] = sotmp[1] ? $.trim(sotmp[1])
							: ts.p.sortorder || "asc"
					}
				}
				...
				var tfoot = "";
				if (ts.p.footerrow) {
					tfoot += "<table role='presentation' style='width:"
						+ ts.p.tblwidth
					...
					.click(
						function (e) {
							...
							if ($.isFunction(ts.p.beforeSelectRow)) {
								var allowRowSelect = ts.p.beforeSelectRow
									.call(ts, ptr[0].id, e);
								if (allowRowSelect === false
									|| allowRowSelect === "stop") {
									cSel = false
								}
							}
					...
					.dblclick(
						function (e) {
							...
							if ($.isFunction(ts.p.ondblClickRow)) {
								dbcr = ts.p.ondblClickRow.call(ts,
									$(ptr).attr("id"), ri, ci,
									e);
								if (dbcr != null) {
									return dbcr
					...
					.bind(
						...
						function (e) {
							...
							if ($.isFunction(ts.p.onRightClickRow)) {
								rcr = ts.p.onRightClickRow.call(ts,
									$(ptr).attr("id"), ri, ci,
									e);
								if (rcr != null) {
									return rcr
				...
				if (ts.p.toolbar[0]) {
					...
					if (ts.p.toolbar[1] === "both") {
						...
						$(grid.uDiv).addClass(
							...
							"t_" + this.id).insertBefore(grid.hDiv);
						$(grid.ubDiv).addClass(
							"ui-userdata ui-state-default").attr("id",
							"tb_" + this.id).insertAfter(grid.hDiv);
						if (hg) {
							$(grid.ubDiv).hide()
						}
					} else {
				...
				if (ts.p.toppager) {
					...
					grid.topDiv = $("<div id='" + ts.p.toppager
						+ "'></div>")[0];
					ts.p.toppager = "#" + ts.p.toppager;
					$(grid.topDiv).addClass(
						"ui-state-default ui-jqgrid-toppager").width(
						grid.width).insertBefore(grid.hDiv);
				...
				if (ts.p.footerrow) {
					...
					hb = $("<div class='ui-jqgrid-hbox"
						+ (dir === "rtl" ? "-rtl" : "") + "'></div>");
					$(grid.sDiv).append(hb).width(grid.width).insertAfter(
						grid.hDiv);
					$(hb).append(tfoot);
		...
		.extend({
			...
			progressBar: function (p) {
				...
				return this.each(function () {
					...
					if (p.htmlcontent !== "") {
						$("#load_" + $.jgrid.jqID(this.p.id)).html(
							p.htmlcontent)
					}
					switch (p.loadtype) {
						case "disable":
											...
											$($t)
												.jqGrid(
													"progressBar",
													{
														method: "show",
														loadtype: $t.p.loadui,
														htmlcontent: $.jgrid.defaults.savetext
														|| "Saving..."
													});
			...
			GridNav: function () {
				return this
					.each(function () {
						...
						$t.p.knv = $t.p.id + "_kn";
						var selection = $("<div style='position:fixed;top:0px;width:1px;height:1px;' tabindex='0'><div tabindex='-1' style='width:1px;height:1px;' id='"
		...
		.extend(
			$.jgrid,
			{
				...
				hideModal: function (selector, o) {
					...
					var thisgrid = o.gb && typeof o.gb === "string"
					&& o.gb.substr(0, 6) === "#gbox_" ? $("#"
						+ o.gb.substr(6))[0] : false;
					if (o.onClose) {
						var oncret = thisgrid ? o.onClose.call(
							thisgrid, selector) : o
							.onClose(selector);
				...
				createModal: function (aIDs, content, p,
					...
					$(mh).append(
						...
						+ p.caption + "</span>");
					var ahr = $(
						"<a class='ui-jqdialog-titlebar-close ui-corner-all'></a>")
						.hover(function () {
				...
				info_dialog: function (caption, content, c_b, modalopt) {
					...
					$("#closedialog", "#info_id").click(
						function () {
							self.hideModal("#info_dialog", {
								jqm: jm,
								onClose: $("#info_dialog").data(
									"onClose")
								|| mopt.onClose,
								gb: $("#info_dialog").data("gbox")
								|| mopt.gbox
							});
							return false
						});
					...
					switch (eltype) {
						...
						case "select":
							...
							if (options.dataUrl !== undefined) {
								var rowid = null, postData = options.postData
									|| ajaxso.postData;
								try {
									rowid = options.rowId
								} catch (e) {
								...
								$
									.ajax($
										.extend(
											{
												url: $
													.isFunction(options.dataUrl) ? options.dataUrl
													.call(
														$t,
														rowid,
														vl,
												...
												success: function (data) {
													var ovm = [], elem = this.elem, vl = this.vl, options = $
														.extend(
															{},
															this.options), msl = options.multiple === true, a = $
														.isFunction(options.buildSelect) ? options.buildSelect
														.call(
															$t,
															data)
														: data;
				...
				checkDate: function (format, date) {
					...
					var daysInMonth = [0, 31, 29, 31, 30, 31, 30, 31,
						31, 30, 31, 30, 31], strDate;
					if (j === -1) {
						return false
					}
				...
				checkValues: function (val, valref, customobject, nam) {
					...
					if (edtrul) {
						if (!nm) {
							nm = g.p.colNames != null ? g.p.colNames[valref]
								: cm[valref].label
						}
						...
						if (edtrul.date === true) {
							if (!(rqfield === false && $.jgrid
									.isEmpty(val))) {
								if (cm[valref].formatoptions
									&& cm[valref].formatoptions.newformat) {
									dft = cm[valref].formatoptions.newformat;
									if ($.jgrid.formatter.date.masks
											.hasOwnProperty(dft)) {
					...
					.each(function () {
						...
						var triggerToolbar = function () {
							...
							$
								.each(
									...
									function () {
										...
										&& $elem.length > 0
										&& $elem[0].nodeName
											.toUpperCase() === "SPAN" ? this.searchoptions.custom_value
											.call(
												$t,
												$elem
							...
							var str = '<ul id="sopt_menu" class="ui-search-menu" role="menu" tabindex="0" style="font-size:'
								...
								+ left
								+ "px;top:"
								+ top + 'px;">', selected = $(elem)
								.attr("soper"), selclass, aoprs = [], ina;
							...
							var cm = $t.p.colModel[i], options = $
								.extend({}, cm.searchoptions);
							if (!options.sopt) {
								options.sopt = [];
								options.sopt[0] = cm.stype === "select" ? "eq"
									: p.defaultSearch
							}
						...
						$
							.each(
								$t.p.colModel,
								function (ci) {
									var cm = this, soptions, surl, self, select = "", sot = "=", so, i, th = $("<th role='columnheader' class='ui-state-default ui-th-column ui-th-"
										+ $t.p.direction
										+ "'></th>"), thd = $("<div style='position:relative;height:auto;padding-right:0.3em;padding-left:0.3em;'></div>"), stbl = $("<table class='ui-search-table' cellspacing='0'><tr><td class='ui-search-oper'></td><td class='ui-search-input'></td><td class='ui-search-clear'></td></tr></table>");
									if (this.hidden === true) {
										$(th).css("display",
											"none")
									}
									...
									if (this.search) {
										if (p.searchOperators) {
											so = (soptions.sopt) ? soptions.sopt[0]
												: cm.stype === "select" ? "eq"
												: p.defaultSearch;
											...
											var st = soptions.searchtitle != null ? soptions.searchtitle
												: p.operandTitle;
											select = "<a title='"
												+ st
										...
										switch (this.stype) {
											...
											case "custom":
												...
												try {
													if ($
															.isFunction(soptions.custom_element)) {
														var celm = soptions.custom_element
															.call(
																$t,
						...
						$(".clearsearchclass", tr)
							.click(
								function (e) {
									var ptr = $(this).parents(
										"tr:first"), coli = parseInt(
										$(
											"td.ui-search-oper",
											ptr).attr(
											"colindex"),
										...
										.extend(
											{},
											$t.p.colModel[coli].searchoptions
			...
			setGroupHeaders: function (o) {
				...
				return this
					.each(function () {
						...
						var ts = this, i, cmi, skip = 0, $tr, $colHeader, th, $th, thStyle, iCol, cghi, numberOfColumns, titleText, cVisibleColumns, colModel = ts.p.colModel, cml = colModel.length, ths = ts.grid.headers, $htable = $(
						...
						if (maxfrozen >= 0 && frozen) {
							...
							if ($t.p.footerrow) {
								var hbd = $(
									...
									+ $.jgrid.jqID($t.p.id))
									.height();
								$t.grid.fsDiv = $('<div style="position:absolute;left:0px;top:'
									+ (parseInt(top, 10)
									+ parseInt(hth, 10)
									+ parseInt(hbd, 10) + 1)
			...
			.each(function () {
				...
				var checkData = function (val, colModelItem) {
					...
					if ($.isFunction(colModelItem.searchrules)) {
						ret = colModelItem.searchrules.call($t, val,
							colModelItem)
					} else {
						if ($.jgrid && $.jgrid.checkValues) {
							try {
								ret = $.jgrid.checkValues.call($t, val, -1,
				...
				this.listernerKeyCode13=function(){
					var fid = "fbox_" + this.p.id;
					$('#'+fid+' .input-elm').on('keypress',function(event){
						if(event.keyCode==13){
							$("#" + fid + "_search").trigger('click');
						}
				...
				this.createTableRowForRule = function (rule, group) {
					var that = this, $t = getGrid(), tr = $("<tr></tr>"), i, op, trpar, cm, str = "", selected;
								...
								$(".input-elm", trpar)
									.bind(
										...
										function (e) {
											...
											&& cm.searchoptions
											&& $
												.isFunction(cm.searchoptions.custom_value) ? cm.searchoptions.custom_value
												.call(
													$t,
													$(
														elem)
					...
					var ruleDataInput = $.jgrid.createEl.call($t,
						cm.inputtype, cm.searchoptions, rule.data,
					...
					if (cm.searchoptions.sopt) {
						...
						if (that.p.sopt) {
							...
							if ($.inArray(cm.searchtype, that.p.strarr) !== -1) {
								op = that.p.stropts
							} else {
								op = that.p.numopts
							}
					...
					for (i = 0; i < op.length; i++) {
						...
						if (ina !== -1) {
							selected = rule.op === that.p.ops[ina].oper ? " selected='selected'"
								: "";
							str += "<option value='" + that.p.ops[ina].oper
								+ "'" + selected + ">"
								+ that.p.ops[ina].text + "</option>"
						}
					}
					...
					$.jgrid.bindEv
						.call($t, ruleDataInput, cm.searchoptions);
					...
					$(ruleDataInput)
						...
						.bind(
							...
							function () {
								...
								rule.data = cm.inputtype === "custom" ? cm.searchoptions.custom_value
									.call(
										$t,
										$(this)
											.children(
				...
				this.getStringForRule = function (rule) {
					var opUF = "", opC = "", i, cm, ret, val, numtypes = [
						"int", "integer", "float", "number", "currency"];
			...
			searchGrid: function (p) {
				...
				return this
					.each(function () {
						...
						var fid = "fbox_" + $t.p.id, showFrm = true, mustReload = true, IDs = {
							themodal: "searchmod" + fid,
							modalhead: "searchhd" + fid,
							modalcontent: "searchcnt" + fid,
							scrollelm: fid
						}, defaultFilters = $t.p.postData[p.sFilter], fl;
						...
						if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
							...
							var columns = $.extend([], $t.p.colModel), bS = "<a id='"
								+ fid
				...
				return this
					.each(function () {
						...
						var gID = $t.p.id, frmgr = "FrmGrid_" + gID, frmtborg = "TblGrid_"
							...
							: false, showFrm = true, maxCols = 1, maxRows = 0, postdata, diff, frmoper;
						frmgr = $.jgrid.jqID(frmgr);
						if (rowid === "new") {
							rowid = ""; //edit by key resource:
							frmoper = "add";
						...
						function createData(rowid, obj, tb, maxcols) {
							var nm, hc, trdata, cnt = 0, tmp, dc, elc, retpos = [], ind = false, tdtmpl = "<td class='CaptionTD'>&#160;</td><td class='DataTD'>&#160;</td>", tmpl = "", i;
							for (i = 1; i <= maxcols; i++) {
								tmpl += tdtmpl
							}
							if (rowid !== "") {
						...
						function postIt() {
							...
							if (ret[0] && !rp_ge[$t.p.id].processing) {
								...
								url = rp_ge[$t.p.id].url
									|| $($t).jqGrid("getGridParam",
										"editurl");
								oper = opers.oper;
								idname = url === "clientArray" ? $t.p.keyName
									: opers.id;
								postdata[oper] = ($
									.trim(postdata[$t.p.id + "_id"]) === "") ? opers.addoper
									: opers.editoper;
						...
						var dh = isNaN(rp_ge[$(this)[0].p.id].dataheight) ? rp_ge[$(this)[0].p.id].dataheight
							: rp_ge[$(this)[0].p.id].dataheight
						+ "px", dw = isNaN(rp_ge[$(this)[0].p.id].datawidth) ? rp_ge[$(this)[0].p.id].datawidth
						...
						var rtlb = $t.p.direction === "rtl" ? true
							: false, bp = rtlb ? "nData" : "pData", bn = rtlb ? "pData"
							: "nData";
						createData(rowid, $t, tbl, maxCols);
						var bP = "<a id='"
							+ bp
							+ "' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>", bN = "<a id='"
							+ bn
							+ "' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>", 
                            // zoufei
                            /*bS = "<a id='sData' class='fm-button ui-state-default ui-corner-all'>"*/
                            bS = "<a id='submitData' class='fm-button ui-state-default ui-corner-all'>"
							+ p.bSubmit + "</a>", bC = "<a id='cData' class='fm-button ui-state-default ui-corner-all'>"
							+ p.bCancel + "</a>";
						...
						var tms = $("<div></div>").append(frm).append(
							bt);
						$.jgrid.createModal(IDs, tms,
							rp_ge[$(this)[0].p.id], "#gview_"
							+ $.jgrid.jqID($t.p.id),
						...
						if (rp_ge[$t.p.id].checkOnSubmit
							|| rp_ge[$t.p.id].checkOnUpdate) {
							bS = "<a id='sNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"
								+ p.bYes + "</a>";
							bN = "<a id='nNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"
								+ p.bNo + "</a>";
							bC = "<a id='cNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"
								+ p.bExit + "</a>";
							var zI = p.zIndex || 999;
							zI++;
							$(
				...
				return this
					.each(function () {
						...
						var rtlb = $t.p.direction === "rtl" ? true
							...
							+ "' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>", bN = "<a id='"
							+ bn
							+ "' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>", bC = "<a id='cData' class='fm-button ui-state-default ui-corner-all'>"
							+ p.bClose + "</a>";
						if (maxRows > 0) {
							var sd = [];
							$.each($(tbl)[0].rows, function (i, r) {
						...
						var bt = $("<div></div>")
							...
							.append(
								...
								+ bC
								+ "</td></tr></tbody></table>");
						$.jgrid.createModal(IDs, bt, p, "#gview_"
							+ $.jgrid.jqID($t.p.id), $("#gview_"
							+ $.jgrid.jqID($t.p.id))[0]);
						if (rtlb) {
					...
					.each(function () {
						...
						if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
							...
							var dh = isNaN(rp_ge[$t.p.id].dataheight) ? rp_ge[$t.p.id].dataheight
								: rp_ge[$t.p.id].dataheight + "px", dw = isNaN(p.datawidth) ? p.datawidth
								: p.datawidth + "px", tbl = "<div id='"
								+ dtbl_id
							...
							tbl += '<tr><td class="delmsg" style="white-space:pre;">'
								...
								+ "</td></tr><tr><td >&#160;</td></tr>";
							tbl += "</tbody></table></div>";
							var bS = "<a id='dData' class='fm-button ui-state-default ui-corner-all'>"
								+ p.bSubmit + "</a>", bC = "<a id='eData' class='fm-button ui-state-default ui-corner-all'>"
								+ p.bCancel + "</a>";
							...
							if (v !== undefined) {
								if (irow === 0) {
									...
									if (typeof v !== "object"
										...
										&& $
											.isFunction(grp.isInTheSameGroup[i]) ? !grp.isInTheSameGroup[i]
											.call(
												$t,
												grp.lastvalues[i],
												v, i, grp)
								...
								.map(
									...
									function (item) {
										if (item.substring(0,
											...
											return parseInt(
												item
													.substring(uid.length + 1),
												10)
										}
									});
							return nums.length > 0 ? nums[0]
								: undefined
						}, itemGroupingLevel, showData, collapsed = false, frz = $t.p.frozenColumns ? $t.p.id
						+ "_frozen"
							: false, tar2 = frz ? $("#"
							+ $.jgrid.jqID(hid), "#"
							+ $.jgrid.jqID(frz)) : false, r2 = (tar2 && tar2.length) ? tar2[0].nextSibling
							: null;
				...
				return this
					.each(function () {
						...
						function buildSummaryTd(i, ik, grp, foffset) {
							var fdata = findGroupIdx(i, ik, grp), cm = $t.p.colModel, vv, grlen = fdata.cnt, str = "", k;
								...
								function (i, n) {
									...
									try {
										if ($
											...
											n.displayValue = grp.formatDisplayField[n.idx]
												.call(
													$t,
													n.displayValue,
													n.value,
									...
									if (grp.groupSummaryPos[n.idx] === "header") {
										...
										str += buildSummaryTd(
											...
											0,
											grp.groups,
											grp.groupColumnShow[n.idx] === false ? (mul === "" ? 2
												: 3)
												: ((mul === "") ? 1
												: 2));
										str += "</tr>"
										...
										str += '<tr id="'
											...
											+ "px;"
											+ '" colspan="'
											+ (grp.groupColumnShow[n.idx] === false ? colspans - 1
												: colspans)
											+ '">'
											+ icon
											+ $.jgrid
									...
									if (leaf) {
										var gg = grp.groups[i + 1], kk, ik, offset = 0, sgr = n.startRow, end = gg !== undefined ? gg.startRow
											: grp.groups[i].startRow
										+ grp.groups[i].cnt;
										if (grp._locgr) {
											offset = (page - 1)
												* rn;
	...
	$.jgrid
		.extend({
			editRow: function (rowid, keys, oneditfunc, successfunc, url,
				...
				return this
					.each(function () {
						...
						bfer = $.isFunction(o.beforeEditRow) ? o.beforeEditRow
							.call($t, o, rowid)
							: undefined;
						if (bfer === undefined) {
							bfer = true
						...
						if (editable === "0"
							...
							if (cnt > 0) {
								...
								$(ind).attr("editable", "1");
								if (o.focusField) {
									if (typeof o.focusField === "number"
										&& parseInt(o.focusField,
											10) <= cm.length) {
										focus = o.focusField
									}
									setTimeout(
			...
			saveRow: function (rowid, successfunc, url, extraparam,
				...
				var bfsr = $.isFunction(o.beforeSaveRow) ? o.beforeSaveRow
					.call($t, o, rowid) : undefined;
				if (bfsr === undefined) {
					bfsr = true
				}
				if (!bfsr) {
				...
				if (editable === "1") {
					...
					if (cv[0] === false) {
						try {
							var tr = $($t).jqGrid("getGridRowById", rowid), positions = $.jgrid
								.findPos(tr);
					...
					if (tmp) {
						...
						if (tmp[idname] === undefined || tmp[idname] === "") {
							...
							if (ind.id !== $t.p.idPrefix + tmp[idname]) {
								var oldid = $.jgrid.stripPref(
									$t.p.idPrefix, rowid);
					...
					if (o.url === "clientArray") {
						...
						success = true;
						$(ind).removeClass("jqgrid-new-row").unbind(
							"keydown")
					} else {
			...
			restoreRow: function (rowid, afterrestorefunc) {
				...
				return this
					.each(function () {
						...
						var bfcr = $.isFunction(o.beforeCancelRow) ? o.beforeCancelRow
							.call($t, o, rowid)
							: undefined;
						if (bfcr === undefined) {
							bfcr = true
			...
			addRow: function (p) {
				...
				return this
					.each(function () {
						...
						var $t = this;
						var bfar = $.isFunction(p.beforeAddRow) ? p.beforeAddRow
							.call($t, p.addRowParams)
							: undefined;
						if (bfar === undefined) {
							bfar = true
						}
								...
								.jqGrid(
									...
									elem,
									{
										...
										onClickButton: function () {
											...
											if (sr) {
												var opers = $t.p.prmNames, oper = opers.oper, tmpParams = o.editParams;
												if ($(
														"#"
														+ $.jgrid
															.jqID(sr),
	...
	$.jgrid
		.extend({
			sortableColumns: function (tblrow) {
				return this
					.each(function () {
						...
						var $e = tblrow.sortable(sortable_opts), dataObj = $e
								.data("sortable")
							|| $e.data("uiSortable");
						if (dataObj != null) {
			...
			columnChooser: function (opts) {
				var self = this, selector, select, colMap = {}, fixedCols = [], dopts, mopts, $dialogContent, multiselectData, listHeight, colModel = self
					.jqGrid("getGridParam", "colModel"), colNames = self
					.jqGrid("getGridParam", "colNames"), getMultiselectWidgetData = function ($elem) {
					return ($.ui.multiselect.prototype && $elem
							.data($.ui.multiselect.prototype.widgetFullName
								...
								return $
									.extend(
										true,
										{
											...
											resize: function () {
												var widgetData = getMultiselectWidgetData(select), $thisDialogContent = widgetData.container
													.closest(".ui-dialog-content");
												if ($thisDialogContent.length > 0
												...
												widgetData.selectedList
													.height(Math
														.max(
															widgetData.selectedContainer
																.height()
															- widgetData.selectedActions
																.outerHeight()
															- 1,
															1));
												widgetData.availableList
													.height(Math
														.max(
															widgetData.availableContainer
																.height()
															- widgetData.availableActions
																.outerHeight()
															- 1,
															1))
											}
							...
							apply_perm: function () {
								...
								$
									.each(
										perm,
										function () {
											delete colMap[colModel[parseInt(
												this,
												10)].name]
										});
				...
				if ($.ui) {
					if ($.ui.multiselect && $.ui.multiselect.defaults) {
						...
						opts.msel_opts = $.extend(
							$.ui.multiselect.defaults, opts.msel_opts)
					}
				}
				if (opts.caption) {
				...
				dopts = $.isFunction(opts.dlog_opts) ? opts.dlog_opts.call(
					self, opts) : opts.dlog_opts;
				call(opts.dlog, selector, dopts);
				mopts = $.isFunction(opts.msel_opts) ? opts.msel_opts.call(
					self, opts) : opts.msel_opts;
				call(opts.msel, select, mopts);
				$dialogContent = $("#colchooser_"
					+ $.jgrid.jqID(self[0].p.id));
				$dialogContent.css({
					margin: "auto"
				});
				...
				multiselectData.availableContainer
					.css({
	...
	$.jgrid
		.extend({
			pivotSetup: function (data, options) {
				var columns = [], pivotrows = [], summaries = [], member = [], labels = [], groupOptions = {
					grouping: true,
					groupingView: {
						groupField: [],
						groupSummary: [],
				...
				this
					.each(function () {
						var row, rowindex, i, rowlen = data.length, xlen, ylen, aggrlen, tmp, newObj, r = 0;
						...
						function agregateFunc(row, aggr, value, curr) {
							...
							for (j = 0; j < jv; j++) {
								...
								for (i = 0; i < arrln; i++) {
									...
									label = !isNaN(parseInt(label, 10)) ? label
									+ " "
										: label;
						...
						xlen = $.isArray(o.xDimension) ? o.xDimension.length
							: 0;
						ylen = o.yDimension.length;
						aggrlen = $.isArray(o.aggregates) ? o.aggregates.length
							: 0;
						if (xlen === 0 || aggrlen === 0) {
						...
						for (i = 0; i < xlen; i++) {
							...
							colc = $
								.extend(true, colc, o.xDimension[i]);
							columns.push(colc)
						}
						...
						while (r < rowlen) {
							...
							do {
								xValue[i] = $
									.trim(row[o.xDimension[i].dataName]);
							...
							if (!newObj) {
								...
								if (ylen >= 1) {
									for (k = 0; k < ylen; k++) {
										yValue[k] = $
											.trim(row[o.yDimension[k].dataName]);
										if (o.yDimension[k].converter
											&& $
												.isFunction(o.yDimension[k].converter)) {
											yValue[k] = o.yDimension[k].converter
												.call(this,
													yValue[k],
													xValue,
													yValue)
									...
									tmp = agregateFunc(row,
										o.aggregates, yValue, tmp)
								} else {
									if (ylen === 0) {
										tmp = agregateFunc(row,
											o.aggregates, null, tmp)
									}
								}
								pivotrows.push(tmp)
							} else {
								...
								if (items.hasOwnProperty(key)) {
									if (typeof items[key] !== "object") {
										if (key === "level") {
											...
											if (lastval[items.level] !== items.text
												...
												if (items.level > 0) {
													...
													var collen = headers[items.level - 1].groupHeaders.length - 1, colpos = collen === 0 ? swaplen
														: initColLen
													+ aggrlen;
													if (items.level - 1 === (o.rowTotals ? 1
															: 0)) {
														if (collen > 0) {
													...
													headers[items.level - 1].groupHeaders[collen].numberOfColumns = columns.length
														- colpos;
										...
										if (items.level > 0) {
											...
											for (l in items.fields) {
												if (items.fields
													...
													if (aggrlen > 1) {
														col.name = l;
														col.label = o.aggregates[j].label
															|| items.label
													} else {
														col.name = items.text;
														col.label = items.text === "_r_Totals" ? o.rowTotalsText
															: items.label
													}
													columns.push(col);
													j++
						...
						if (groupfields > 0) {
							for (i = 0; i < groupfields; i++) {
								if (columns[i].isGroupField) {
									groupOptions.groupingView.groupField
										.push(columns[i].name);
									groupOptions.groupingView.groupSummary
										.push(o.groupSummary);
									groupOptions.groupingView.groupSummaryPos
										.push(o.groupSummaryPos)
								}
							}
						} else {
			...
			jqPivot: function (data, pivotOpt, gridOpt, ajaxOpt) {
				return this
					.each(function () {
						...
						function pivot(data) {
							var pivotGrid = jQuery($t).jqGrid(
								"pivotSetup", data, pivotOpt), footerrow = $
								.assocArraySize(pivotGrid.summary) > 0 ? true
								: false, query = $.jgrid
								.from(pivotGrid.rows), i;
			...
			setSubGrid: function () {
				return this.each(function () {
					...
					$t.p.colModel.unshift({
						name: "subgrid",
						width: $.jgrid.cell_width ? $t.p.subGridWidth
						+ $t.p.cellLayout : $t.p.subGridWidth,
						sortable: false,
						resizable: false,
						hidedlg: true,
						search: false,
												...
												.extend(
													{
														type: ts.p.mtype,
														url: $
															.isFunction(ts.p.subGridUrl) ? ts.p.subGridUrl
															.call(
																ts,
																dp)
															: ts.p.subGridUrl,
	...
	$.jgrid
		.extend({
					...
					.each(function () {
						...
						switch ($t.p.treeGridModel) {
							case "nested":
								...
								break;
							case "adjacency":
								var parent_id = $t.p.treeReader.parent_id_field, dtid = $t.p.localReader.id, ind = rc[dtid], pos = $t.p._index[ind];
								while (pos--) {
									if ($t.p.data[pos][dtid] === $.jgrid
											.stripPref($t.p.idPrefix,
												rc[parent_id])) {
										result = $t.p.data[pos];
										break
			...
			isNodeLoaded: function (rc) {
				...
				this
					.each(function () {
						...
						var isLeaf = $t.p.treeReader.leaf_field, loaded = $t.p.treeReader.loaded;
						if (rc !== undefined) {
				...
				return this
					.each(function () {
						...
						if (!rc[expanded]) {
							...
							var rc1 = $("#" + this.p.idPrefix
								+ $.jgrid.jqID(id), this.grid.bDiv)[0];
			...
			addChildNode: function (nodeid, parentid, data, expandData) {
				var $t = this[0];
				if (data) {
					var expanded = $t.p.treeReader.expanded_field, isLeaf = $t.p.treeReader.leaf_field, level = $t.p.treeReader.level_field, parent = $t.p.treeReader.parent_id_field, left = $t.p.treeReader.left_field, right = $t.p.treeReader.right_field, loaded = $t.p.treeReader.loaded, method, parentindex, parentdata, parentlevel, i, len, max = 0, rowind = parentid, leaf, maxright;
...
(function ($) {
	...
	var s = 0, H = $.jqm.hash, A = [], F = false, e = function (h) {
		f(h)
	}, f = function (h) {
		try {
...
(function ($) {
	...
	$.fn.fmatter.select = function (cellval, opts) {
		...
		if (opts.colModel.formatoptions !== undefined) {
			...
			if (opts.colModel.editoptions !== undefined) {
		...
		if (oSelect) {
			var msl = (opts.colModel.editoptions != null && opts.colModel.editoptions.multiple === true) === true ? true
				: false, scell = [], sv;
			if (msl) {
				scell = cellval.split(",");
	...
	$.fn.fmatter.rowactions = function (act) {
		var $tr = $(this).closest("tr.jqgrow"), rid = $tr.attr("id"), $id = $(
			this).closest("table.ui-jqgrid-btable").attr("id").replace(
			/_frozen([^_]*)$/, "$1"), $grid = $("#" + $id), $t = $grid[0], p = $t.p, cm = p.colModel[$.jgrid
			.getCellIndex(this)], $actionsDiv = cm.frozen ? $("tr#" + rid
			+ " td:eq(" + $.jgrid.getCellIndex(this) + ") > div", $grid)
			: $(this).parent(), op = {
			extraparam: {}
	...
	$.unformat = function (cellval, options, pos, cnt) {
		...
		if (unformatFunc !== undefined && $.isFunction(unformatFunc)) {
			...
			if (formatType !== undefined && $.fmatter.isString(formatType)) {
				...
				switch (formatType) {
					...
					case "currency":
						...
						ret = ret.replace(stripTag, "").replace(
							op.decimalSeparator, ".");
						break;
					case "checkbox":
	...
	$.unformat.select = function (cellval, options, pos, cnt) {
		...
		var op = $
			.extend(
				{},
				options.colModel.formatoptions !== undefined ? options.colModel.formatoptions
		...
		if (op.value) {
			...
			if ($.fmatter.isString(oSelect)) {
				...
				if ($.fmatter.isObject(oSelect) || $.isArray(oSelect)) {
					if (!msl) {
						scell[0] = cell
					}
					ret = $.map(scell, function (n) {
				...
				if (o === null) {
					...
					if (typeof (o) === "object") {
						...
						json += (name ? ":{" : "{")
							...
							+ arr.join("," + newline + ind + tab)
							+ newline + ind) : arr.join("")) + "}"
					} else {
						if (typeof (o) === "string") {
							json += (name && ":")
								+ '"'
