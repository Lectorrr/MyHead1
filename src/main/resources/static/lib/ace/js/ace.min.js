(function($, undefined) {
	...
	ace.vars["ios_safari"] = !!agent.match(/OS ([4-9])(_\d)+ like Mac OS X/i)
			&& !agent.match(/CriOS/i);
	ace.vars["ie"] = window.navigator.msPointerEnabled
...
jQuery(function($) {
	...
	enableAjax(mPrefix,mDefaultUrl);
	handleScrollbars();
	dropdownAutoPos();
	navbarHelpers();
	sidebarTooltips();
	...
	function enableSidebar() {
		...
		if ($.fn.ace_sidebar_scroll) {
			$sidebar.ace_sidebar_scroll({
				...
				"include_toggle" : false || ace.vars["safari"]
						|| ace.vars["ios_safari"],
				"smooth_scroll" : 150,
				"outside" : false
			});
	...
	function enableAjax(mPrefix,mDefaultUrl) {
		// Kevin 这里定义一个前缀的变量，如果需要更改首页地址，那么请给这个变量赋值
		if(!mPrefix){
			// 如果没有定义该参数，默认为 sys/index
			mPrefix = "sys/index";
		}
		if(!mDefaultUrl){
			mDefaultUrl = "page/sys/home";
		}
		if ($.fn.ace_ajax) {
			$("[data-ajax-content=true]").ace_ajax(
					{
						...
						"content_url" : function(hash) {
							...
							if (path.match(mPrefix)) {
								if (hash.match(/http:/)) {
									return hash.replace(/^page\//, "").replace(/^ifContent\//, "");
								}
								return path.replace(mPrefix, hash.replace(/^page\//, "").replace(/^ifContent\//, ""));
							}
							var url = path + "?" + hash.replace(/\//, "=");
							return url;
						},
						"default_url" : mDefaultUrl
					});
		}
	}
	function handleScrollbars() {
		...
		if (has_scroll && !ace.vars["old_ie"]) {
			$(window).on(
					"resize.reset_scroll",
					function() {
						$(".ace-scroll:not(.scroll-disabled)").not(":hidden")
								.ace_scroll("reset")
					});
			if (has_scroll) {
				$(document).on(
						"settings.ace.reset_scroll",
						function(e, name) {
							if (name == "sidebar_collapsed") {
								$(".ace-scroll:not(.scroll-disabled)").not(
										":hidden").ace_scroll("reset")
							}
						})
			}
		}
		...
		$(document)
				.on(
						...
						function() {
							...
							if (parseInt(offset.top + $(this).height()) + 50 > (ace.helper
									.scrollTop()
									+ ace.helper.winHeight() - parent.find(
									".dropdown-menu").eq(0).height())) {
	...
	function navbarHelpers() {
		$('.ace-nav [class*="icon-animated-"]').closest("a").one(
				"click",
				function() {
					var icon = $(this).find('[class*="icon-animated-"]').eq(0);
					var $match = icon.attr("class").match(
							/icon\-animated\-([\d\w]+)/);
		...
		$(document).on(
				...
				function(e) {
					...
					var $this, href;
					var that = e.target;
					if (($this = $(e.target).closest("[data-toggle=tab]"))
							&& $this.length > 0) {
						$this.tab("show");
						e.preventDefault();
	...
	function sidebarTooltips() {
		$(".sidebar .nav-list .badge[title],.sidebar .nav-list .badge[title]")
				.each(
						function() {
							var tooltip_class = $(this).attr("class").match(
									/tooltip\-(?:\w+)/);
	...
	function scrollTopBtn() {
		...
		if (scroll_btn.length > 0) {
			...
			scroll_btn.on(ace.click_event, function() {
				var duration = Math.min(500, Math.max(100, parseInt(ace.helper
						.scrollTop() / 3)));
				$("html,body").animate({
					scrollTop : 0
				}, duration);
	...
	function someBrowserFix() {
		...
		if (ace.vars["ios_safari"]) {
			$(document).on(
					"ace.settings.ios_fix",
					function(e, event_name, event_val) {
						if (event_name != "navbar_fixed") {
							return;
						}
						$(document).off("focus.ios_fix blur.ios_fix",
	...
	function smallDeviceDropdowns() {
		...
		$(".ace-nav > li").on("shown.bs.dropdown.navbar", function(e) {
		...
		function adjustNavbarDropdown() {
			...
			if ($sub.css("position") == "fixed") {
				...
				var offset_w = win_width > 320 ? 60 : (win_width > 240 ? 40
						: 30);
				...
				var tabbed = false;
				var extra_parts = 0;
				var dropdown_content = $sub
						.find(".tab-pane.active .dropdown-content.ace-scroll");
				if (dropdown_content.length == 0) {
					dropdown_content = $sub
							.find(".dropdown-content.ace-scroll")
				} else {
					tabbed = true
				}
				...
				if (dropdown_content.length == 1) {
					...
					extra_parts += parent_menu.find(".dropdown-header")
							.outerHeight();
					extra_parts += parent_menu.find(".dropdown-footer")
							.outerHeight();
					var tab_content = parent_menu.closest(".tab-content");
					if (tab_content.length != 0) {
						extra_parts += tab_content.siblings(".nav-tabs").eq(0)
								.height()
					}
				}
				var height = parseInt(Math.min(avail_height, 480, scrollHeight
						+ extra_parts));
				var left = parseInt(Math
						.abs((avail_width + offset_w - width) / 2));
				...
				if (dropdown_content.length == 1) {
					if (!ace.vars["touch"]) {
						...
						dropdown_content.ace_scroll("disable").css(
								"max-height", height - extra_parts).addClass(
								"overflow-scroll")
					}
				}
				...
				$(this).closest(".navbar.navbar-fixed-top").css("z-index",
						zindex)
			} else {
				if ($sub.length != 0) {
			...
			var self = this;
			$(window).off("resize.navbar.dropdown").one(
					"resize.navbar.dropdown", function() {
		...
		function resetNavbarDropdown($sub) {
			...
			if ($sub.length > 0) {
				$sub.css({
					...
					"right" : "",
					"top" : ""
				}).find(".dropdown-content").each(
						function() {
							if (ace.vars["touch"]) {
								$(this).css("max-height", "").removeClass(
										"overflow-scroll")
							}
				...
				if ($sub.hasClass("user-menu")) {
					var user_info = $(this).find(".user-info").css({
						"left" : "",
						"right" : "",
						"top" : "",
						"width" : "",
...
ace.helper.scrollTop = function() {
	return document.scrollTop || document.documentElement.scrollTop
...
ace.helper.removeStyle = "removeProperty" in document.documentElement.style ? function(
		elem, prop) {
...
ace.helper.hasClass = "classList" in document.documentElement ? function(elem,
		className) {
